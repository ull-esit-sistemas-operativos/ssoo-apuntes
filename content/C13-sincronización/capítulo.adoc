...
Además, en el <<_hilos>> vimos que en los procesos multihilo todos los hilos comparten el espacio de direcciones del proceso al que pertenecen, lo que significa que puedan acceder al mismo tiempo a variables globales y a la memoria reservada dinámicamente.
...
. Extrae el `item` del campo que lo contienen en el nodo.
. Destruir el nodo.
. Salir del método retornado el elemento.
...
Los *semáforos* son un tipo de objetos del sistema operativo que nos permite controlar el acceso a una sección crítica, por medio de dos primitivas: *acquire* y *release* —o *wait* y *signal*, según el libro de texto que consultemos—.
...
Mientras tanto `release()` retorna y la ejecución del hilo que sale del sección crítica continua.
...
* Los *semáforos anónimos*((semáforo, anónimo))) que sólo existen en el espacio de direcciones del proceso que las crea, de tal forma que están disponibles para sincronizar hilos del mismo procesos.
+
...
En el ejemplo {anom_shared_memory_cpp} de comunicación mediante memoria compartida, se usa un *semáforo* para que el proceso hijo indique al procesos padre que ha terminado del calcular el factorial y el resultado ya está en la memoria.
...
Para enviar los mensajes entre el cliente y el servidor, en la memoria compartida se reserva hueco para un _buffer_ en el que el cliente copia el comando que quiere enviar y para dos *semáforos*:
...
----
<1> Indica cuándo `command_buffer` está vacío, así que se inicializado a 1.
El cliente usa {linux_sem_wait} en este *semaforo* antes de escribir un nuevo comando en `command_buffer`:
...
* Si el *semáforo* está a 1, el servidor queda bloqueado y tiene que espera a que el cliente use {linux_sem_post} sobre el mismo *semáforo*.
...
Finalmente, pare resolver el *problema del productor-consumidor* tenemos que considerar que:
...
<5> Antes de inserta un elemento se decrementa `empty_count`.
...
Lo interesante es que a la inversa ocurre igual.
Si un sistema o lenguaje soporta *_mutex_*, es muy sencillo hacer una implementación *semáforos*.
Esto, por ejemplo, es lo que ocurre en {cpp}, donde el estándar no soporta *semáforos* pero su implementación usando {cpp_mutex} es muy simple, como se puede ver en el <<cpp_semáforo>>.

El estándar POSIX soporta *_mutex_* a través de {pthreads}.
Por defecto solo se pueden utilizar para sincronizar hilos del mismo proceso; pero tienen un atributo para permitir la sincronizar entre procesos diferentes, aunque para eso deben ser creados en una región de memoria compartida por dichos procesos.
...
En la solución que dimos al *problema del productor-consumidor* usando *semáforos* (véase el <<_ejemplos_del_uso_de_semáforos>>) empleamos *semáforos* para implementar las esperas del productor y el consumidor cuando el vector está lleno o vacío, respecitvamente.
...
El hilo debe haber adquirido antes *_mutex_*, es liberado en el momento de poner al hilo en estado *esperando*.
...
Por defecto, las *variables de condición* de {pthreads} solo se pueden utilizar para sincronizar hilos del mismo proceso; pero tienen un atributo para permitir la sincronizar entre procesos diferentes.
...
* El hilo puede iterar comprobado constantemente la condición hasta que se cumple.
A esa técnica se la denomina *((espera ocupada))*.
...
Para evitar que las esperas ocupadas sean demasiado largas, los sistema operativos nunca expulsan de la CPU a hilos que se estén ejecutando dentro de secciones críticas controladas por objetos de sincronización con este tipo de espera, con la idea de que salgan den la sección crítica los antes posible.
