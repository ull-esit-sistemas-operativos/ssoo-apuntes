...
Entonces, el *tiempo de acceso efectivo* se podría calcular como la probabilidad (1 - _p_) de que la entrada no esté en la *TLB*, por el *tiempo de acceso* necesario, en ese caso 2__T~m~__, más la probabilidad de que la entrada sí esté en la *TLB* _p_, por el *tiempo de acceso* _T~m~_, porque solo hace falta acceder una vez a la memoria:
...
Compartir *páginas* no solo permite ahorrar memoria, pues en los sistemas operativos modernos, la comunicación entre procesos mediante memoria compartida (véase el <<_memoria_compartida>>), se implementa mediante *páginas compartidas*.
...
Si organizamos estas 1024 entradas en un vector lineal, obtendremos una *tabla de páginas externa*(((tabla, páginas, externa))) de 4 KiB (véase la <<paginación_jerárquica>>).
