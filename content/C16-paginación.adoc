...
El stem:[T_text(em)] del primer caso suele ser mucho mayor que el segundo porque, por lo general, stem:[T_text(TLB) ll T_text(m)].
...
Como comentamos anteriormente, esta expresión se puede simplificar si consideramos que stem:[T_text(TLB) ll T_text(m)]:
...
Esta expresión se puede simplificar si consideramos que stem:[T_text(TLB) ll T_text(m)]:
