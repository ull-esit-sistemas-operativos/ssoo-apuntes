...
Si usamos este tipo de sistema pero es importante conservar la separación entre los mensajes recibido, será nuestra responsabilidad escoger un formato de mensaje adecuado que permita al receptor recuperar dónde comienza y termina un mensaje dentro de la secuencia de bytes.

== Referenciación

Los procesos que se quiera comunicar debe tener una forma de señalarse el uno al otro.
Para ello el diseñador del sistema puede elegir que el sistema de paso de mensajes se con comunicación directa o la indirecta.
...
* *send( A, message )* para mandar un mensaje al proceso identificado como «A»
* *receive( &pid, &message )* para recibir un mensaje de cualquier proceso, recibiendo en «_message_» la una copia del _message_ y en «pid» la identidad del remitente.

====
En resumen:

* En el *direccionamiento simétrico* tanto el proceso que envía como el que reciben tienen que identificar al otro para comunicarse.
...
Por tanto, los procesos sólo necesitan conocer a la identidad de los otros para comunicarse.
...
La principal desventaja de este tipo de comunicación es que si cambia el identificador de un proceso hay que actualizar todas las referencias en todos los procesos que se comunican con el.
...
Incluso puede darse el caso que tengan conexión con otros procesos y que quiera aprovecha para intentar comunicarse con alguno de ellos.
...
Si se llamada a `send()` cuando la cola de mensajes esté llena, lo más común es que retorne con un código de retorno que indique que el proceso debe volver a intentar el envío más tarde.
...
Sin embargo, cada proceso puede declarar un *((manejador de señal))*.
...
Un extremo permite los procesos en ese extremo escribir en la tubería, mientras el otro extremo permite a los procesos leer de la tubería los datos escritos desde el otro extremo.

// TODO: Esquema de una tubería.

Existen dos tipos de tuberías:

* Las *tuberías anónimas* sólo existen en el espacio de direcciones del proceso que las crea, de tal forma que debe heredarse de padres a hijos para que otros procesos puedan tener acceso.

* Las *tuberías con nombre* son públicas al resto del sistema, por lo que teóricamente cualquier proceso con permisos puede abrir una para comunicarse con otros procesos.
...
| Crear tubería anónima         | {linux_pipe}      | {win32_createpipe}
| Crear tubería con nombre      | {linux_mkfifo}    | {win32_createnamedpipe}
| Abrir tubería con nombre      | {linux_open}      | {win32_createfile}
| Leer                          | {linux_read}      | {win32_readfile}
| Escribir                      | {linux_write}     | {win32_writefile}
| Cerrar                        | {linux_close}     | {win32_closehandle}
|====
...
En {forkpipe_c} se puede observar un ejemplo de esto.
...
En {forkredir_c} se puede ver un ejemplo de como ejecutar el comando `ls` y redirigir su salida al proceso padre para contar el número de líneas en lo que eñ comando quería mostrar por pantalla.
...
La dificultad es que cada familia de protocolos tiene un formato de direcciones diferente, así que hay que tener cuidado de usar le adecuado:
...
----
<1> Como en el esto de llamadas al sistema, en caso de error se devuelve un número negativo y {linux_errno} contendrá el código del error.
...
{linux_sendto} sólo se puede utilizar con _sockets_ conectados.
...
También se pueden utilizar las funciones {linux_select} y {linux_poll} para monitorizar varios _sockets_ al mismo tiempo, de forma similar a como se hace para colas de mensajes POSIX (véase el <<_colas_de_mensages_posix>>).
