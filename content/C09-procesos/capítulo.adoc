...
Como ya hemos comentado con anterioridad, un *proceso* es un programa en ejecución (véase el <<_gestión_de_procesos>> para una definición más completa).
...
Como el *contador de programa*, los valores de los *registros de la CPU*, el *estado* del proceso y más (véase el <<_bloque de control de proceso>>).
====
...
[NOTE]
====
Como vimos en el <<_gestión_de_procesos>> varios procesos pueden estar asociados al mismo programa pero no por eso dejan de ser distintos procesos.
Todos tendrá una copia del mismo segmento de código, pero diferente: contador de programas, valores en los registros de la CPU, pila, segmento de datos, montón y demás propiedades.
====

// TODO: Subir código (no pegado abajo) y bajar la pila o crear otra imagen para el caso de Linux.
En la <<proceso_en_memoria>> se puede observar la disposición de algunos de estos elementos de un proceso en la memoria.

[[proceso_en_memoria]]
.Anatomía de un proceso en memoria.
image::proceso_en_memoria.svg[]

// TODO: Librerías compartidas
...
[[diagrama_estado_proceso]]
.Diagrama de estado de un proceso.
image::diagrama_estado_proceso.svg[]
...
Necesita de varias operaciones que pueden tarda tiempo en realizarse, como: reservar memoria libre, cargar el programa en la memoria, inicializar estructuras de datos y configurar el entorno de ejecución.
...
Sólo puede haber un proceso en este estado por CPU en el sistema.
...
El diagrama de estados de los procesos, con las transiciones posibles entre ellos, se muestra en la <<diagrama_estado_proceso>>.
...
Contiene a todos los trabajos en el sistema, de manera que cuando entran en el sistema van a esta cola, a la espera de ser escogidos para ser cargados en la memoria y ejecutados.
...
Una manera habitual de representar la planificación de procesos es a través de un diagrama de colas como el de la <<colas_de_planificación_procesos>>.

// TODO: Buscar una forma de mostrar este diagrama de forma animada.
// TODO: Marca el nuevo y terminado.

[[colas_de_planificación_procesos]]
.Diagrama de colas de la planificación de procesos.
image::colas_planificación_procesos.svg[]
...
* El *planificador de largo plazo*(((planificador, largo plazo))) Do *planificador de trabajos*(((planificador, trabajos)))— selecciona los trabajos desde la cola de trabajos en el almacenamiento secundario —dónde están todos almacenados— y los carga en memoria.
+
Este planificador se usaba en los sistemas multiprogramados, donde había cola de trabajo.
...
El *((cambio de contexto))* es la tarea de asignar la CPU un proceso distinto al que la tiene asignada en el momento actual.
...
Esto permite que la CPU almacene de forma eficiente el valor de los registros de más de un proceso, sin que en cada cambio de contexto sea necesario copiarlos al PCB del proceso en la memoria principal.
====
...
// TODO: Poner una captura de pstree

En estos sistemas el proceso *((init))* es el proceso padre raíz de todos los procesos de usuario.
...
. Que cada proceso hijo puede solicitar y obtener los recursos directamente del sistema operativo, compitiendo por los recursos del sistema en las mismas condiciones que el resto de procesos en ejecución.
Esta es la opción más común en los sistemas de propósito general actuales, como Microsoft Windows, Android, Linux, macOS, BSD y muchos otros.

. Que los proceso hijo solo puedan aspirar a obtener un subconjunto de los recursos de su padre.
...
En este último caso, el proceso padre puede estar obligado a repartir sus recursos entre los procesos hijo. O pueda que el sistema les permita compartir algunos de esos recursos —como memoria o archivos— con algunos de sus hijos.
...
| Leer              | {clang_getenv}    | {win32_getenvironmentvariable}
| Leer todos        | {clang_environ}   | {win32_getenvironmentstrings}
| Crear / modificar | {clang_setenv}    | {win32_setenvironmentvariable}
|====
...
Algunos ejemplo podrían ser: dispositivos de E/S, memoria compartida, tuberías, _sockets_ y otros mecanismos de comunicación. 

En POSIX todo proceso tiene, por defecto, tres archivos abiertos que corresponden a tres dispositivos de E/S especiales:

 * *Entrada estándar*(((entrada estándar))), de dónde los procesos leen la entrada del teclado de la terminal.
 * *Salida estándar*(((salida estándar))), donde el proceso escribe para mostrar texto en la pantalla de la terminar.
...
. Que le padre quede detenido a la espera que de algunos o todos sus hijos terminen.
Era lo más frecuente en sistemas monotarea, como {msdos}.

==== Cómo se construye el espacio de dirección de los procesos hijo
...
Esto lo veremos con más detalle en el <<_ejemplos_de_operaciones_con procesos>>.
...
|====
|                               | POSIX API         | Windows API
| Salir                         | {clang_exit}      | {win32_exitprocess}
| Esperar (un hijo concreto)    | {linux_waitpid}   | {win32_waitforsingleobject}
| Esperar (múltiples hijos)     | {linux_wait}      | {win32_waitformultipleobjects}
| Terminar otro proceso         | {linux_kill}      | {win32_terminateprocess}
|====

En todo caso un proceso puede provocar la terminación de otro proceso a través de una llamada al sistema.
Por ejemplo, en sistemas POSIX se usa un mecanismo llamado *((señales))*:
...
Los motivos para terminar un procesos hijo pueden ser:
...
En el <<ejemplo_createprocess>> se puede ver como se usa {win32_createprocess} para ejecutar un programa y esperar a que termine, de forma similar a como lo hace {clang_system}.

[[ejemplo_createprocess]]
...
<2> {win32_processinformation} sirve para devuelve el manejador y el *identificador de proceso* del nuevo proceso.
...
Con `NULL` indicamos que utilice las misma ruta que el proceso padre.
...
<11> Cuando el proceso a terminado, el padre puede conocer su valor de salida.
...
<12> Cuando ya no hace falta obtener información del procesos hijo o manipularlo, es necesario cerrar los manejadores devueltos por {win32_createprocess}.
...
Los nuevos procesos se crean con la llamada {linux_fork}, que se encargar de crearlo como una copia del proceso padre.
...
+
Debemos asegurarnos de llamar a {linux_wait} o {linux_waitpid} una vez por cada proceso hijo, en algún momento, porque así es como el sistema sabe que el padre ya no tiene más interés en el proceso y puede liberar su PCB, dónde se guarda el estado de salida.
No hacerlo genera *procesos zombi*(((proceso, zombi)) o _defunct_.
<9> El valor de salida de proceso hijo lo obtiene el proceso a través del estado de salida devuelto por {linux_wait}.
...
Lo siguiente es un posible resultado de ejecutar el programa anterior en una terminal de Linux,
numerado con las anotaciones realizadas al código:
...
Es decir, después del {linux_fork} ambos procesos son complemente independientes.
...
Para eso necesitamos {linux_exec}, una familia de funciones cuyo propósito es cagar un nuevo programa en el proceso que la invoca.
...
<2> En la rama de código que se va a ejecutar en el hijo —gracias a la comprobación del valor devuelto por {linux_fork}— ejecutamos al función de la familia {linux_exec} que más nos interese.
...
<7> Si la función {linux_exec} la ejecución del proceso hijo continuará, hasta salir de {clang_main} pero, generalmente, el proceso ya no es útil si no puede ejecutar el programa que le hemos indicado.
...
Veamos qué ocurre si línea de la función {linux_exec} fuera:
...
<2> Por eso el programa, al terminar el proceso, no devuelve 0 sino 2 y es ese el valor que recibe el proceso padre.
...
[HIJO] ¡Soy el proceso hijo!
[HIJO] Voy a ejecutar el comando 'ls'
Error (2) al ejecuta el programa: No such file or directory <1>
...
Si el primero argumento contienen alguna '/', se considera una ruta y se busca directamente el ejecutable en ella.
...
Es importante destacar que la mejora en la velocidad sólo es posible si el sistema tiene varios componentes de procesamiento como procesadores —si se quiere acelerar la ejecución en la CPU— o canales E/S —si se quieren acelerar las operaciones de E/S —.
...
Las ejecución simultánea de procesos cooperativos requiere mecanismos tanto para comunicar unos con otros como para sincronizar sus acciones (véase el <<_sincronización>>).
...
En la <<modelos_de_comunicación>> se puede un esquema comparativo entre ambos modelos de comunicación.
Veremos cada uno en detalle en el <<_memoria_compartida>> y el <<_paso_de_mensages>>, respectivamente.

[[modelos_de_comunicación]]
.Modelos de comunicación.
image::modelos_comunicación.svg[]
