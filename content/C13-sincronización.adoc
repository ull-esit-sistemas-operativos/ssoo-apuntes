...
Pero el único resultado correcto es 5, que es el que obtendremos si ejecutamos las sentencias secuencialmente, sin mezclar las operaciones en las que se dividen.
...
(((instrucción, atómica)))
Todas las CPU modernas disponen de instrucciones para comparar y modificar el contenido de una variable o intercambiar el contenido de dos variables, de forma *((atómica))*. 
...
Los semáforos inicializados a 1 se denominan *_((mutex))_* —término que tiene su origen en _mutual exclusion_— o *semáforos binarios*(((semáforo, binario))).
