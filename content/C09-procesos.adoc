...
El segmento BSS —siglas de _block started by symbol_— contiene las variables globales y estáticas del programa inicializadas a 0 o sin inicialización explícita.
...
+
En el esquema de la <<fig-proceso-en-memoria>>, este segmento suele ir junto al de datos.
...
image::{chapdir}/diagrama_estado_proceso.svg[pdfwidth="60iw"]
...
image::{chapdir}/colas_planificación_procesos.svg[pdfwidth="77.6iw"]
...
==== Cómo obtienen los procesos hijos los recursos que necesitan

Hay varios aspectos en la creación de los procesos que pueden variar de un sistema operativo a otro.
Uno de ellos es cómo obtienen los procesos hijos los recursos que necesitan para hacer su trabajo.
...
<9> El valor de salida del proceso hijo lo obtiene el proceso padre a través del estado de salida devuelto por {linux_wait}.
...
La ejecución simultánea de procesos cooperativos requiere mecanismos tanto para comunicar unos con otros como para sincronizar sus acciones (véase el <<sincronización>>).
...
Veremos cada uno en detalle en el <<memoria_compartida>> y el <<comunicación_mediante_paso_de_mensajes>>, respectivamente.

[[fig-modelos-de-comunicación]]
.Modelos de comunicación.
image::{chapdir}/modelos_comunicación.svg[pdfwidth="77.6iw"]
