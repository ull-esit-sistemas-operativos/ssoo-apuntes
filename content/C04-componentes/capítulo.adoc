...
Cada sistema operativo diferentes componentes con distinto nombre.
...
+
Los procesos son entidades activas que necesita recursos —CPU, memoria, archivos, dispositivos E/S.
...
El contenido de un archivo en disco con las instrucciones que algún día una CPU ejecutará.
...
Por el momento estamos considerando que *proceso* y *trabajo* hacen referencia al mismo concepto porque en los sistemas más antiguos (véase el <<_mainframe>>) la unidad de trabajo se llamaba *trabajo* mientras que en los sistemas modernos se llama *proceso*, de tal forma que podemos considera al segundo una evolución del primero.

Sin embargo, mirándolo exclusivamente desde la perspectiva de los sistemas operativos modernos son dos conceptos diferentes, puesto que un trabajo puede ser realizado por un solo proceso o mediante la colaboración de varios procesos.

=== Responsabilidades de la gestión de procesos

El componente de gestión de procesos es el responsable de la siguientes actividades:
...
[NOTE]
====
Una característica de los sistemas UNIX es que todos los dispositivos de E/S se representa como un archivo en el sistema de archivos.
Esto se puede comprobar rápidamente visitando el directorio path::/dev[] en cualquier sistema GNU/Linux o BSD, ya que es allí donde suelen estar.

Así no hace falta diseñar y aprender una interfaz diferente para cada tipo de dispositivo.
Los procesos que quiere utilizar cualquier dispositivos de E/S solo tienen que usar las mismas funciones y llamadas al sistema que emplean para manipular los archivos normales.
Por ejemplo, {sys_open}, {sys_read}, {sys_write} o {sys_close}.
====

=== ((Buffering))

El *buffering* o uso de memoria intermedia es una estrategia en la que se almacenan los datos de manera temporal en una zona de la memoria, llamada _buffer_.

Consiste en que el controlador indica a un dispositivo que escriba los bloques de datos solicitados en un _buffer_.
Cuando la escritura del _buffer_ se ha completado, se transfiere su contenido al proceso que hizo la solicitud para que procese los datos.
Mientras lo hace, el controlador indica al dispositivo que copie nuevos datos en el _buffer_.

Por ejemplo, al grabar sonido del dispositivo de sonido del sistema no se entregan las muestras una a una al proceso.
En su lugar se graban varios miles de muestras que se escriben en un _buffer_.
Cuando el _buffer_ está lleno, se transfieren todas las muestras al proceso de una sola vez y se siguen grabando muestras en el _buffer_.

Lo mismo ocurre al reproducir sonido.
El proceso no entrega las muestras de sonido de una en una al dispositivo, sino que empaqueta varias miles que se copian al _buffer_ de una sola vez.
Entonces el controlador indica al dispositivo que lea las muestras desde ese _buffer_ según lo vaya necesitando.
...
La memoria principal es de tamaño limitado, por lo que sólo se mantiene copia de los datos utilizados con mayor frecuencia.

=== ((Spooling))

El *spooling* se utiliza en dispositivos que no admiten el acceso simultaneo de varias aplicaciones a vez, como es el caso de impresoras y unidades de cinta.
...
Los programas que se desean ejecutar deben estar en la memoria principal —o almacenamiento primario— pero ésta es demasiado pequeña para alojar todos los datos y todos los programas del sistema.
...
* Planificar el acceso a los dispositivos, de tal forma que se orden las operaciones de forma eficiente.
...
El acceso a cada tipo medio es controlado por un dispositivo —por ejemplo el controlador de disco o la unidad de DVD-ROM— que también tiene características propias.
...
Para simplificar aun más el acceso al almacenamiento, el sistema operativo proporciona una visión lógica uniforme de todos los sistemas de almacenamiento.
Es decir, abstrae las propiedades físicas de los dispositivos de almacenamiento para definir el *archivo*, una unidad de almacenamiento lógico con la que trabajan los procesos para guarda y recuperar datos.
...
El sistema de archivos utiliza al gesto del almacenamiento secundario y al sistema de E/S y es responsable de la siguientes actividades:
...
* Son mecanismos necesarios cuando un sistema informático tiene múltiples usuarios y permite la ejecución concurrente de varios procesos, pues así sólo pueden utilizar los recursos aquellos procesos que hayan obtenido la autorización del sistema operativo.
...
Mientras que en algunos sistemas se pueden establecer permisos sobre los archivos para garantizar que sólo los procesos con la debida autorización tengan acceso.
