...
// TODO: Esta nota puede ser un sidebar sobre  anillos en Intel, modo real y modo privilegiado.

[NOTE]
====
Aunque para operar en modo dual solo se necesita que la CPU admita los dos modos descritos, existen procesadores que soportan más, con la idea de que cada uno sirva para ejecutar código con diferente nivel de privilegio.

Por ejemplo, la arquitectura Intel x86 soporta 4 modos.
Sin embargo, los sistemas operativos con mayor cuota de mercado —incluyendo Microsoft Windows, macOS, Linux y Android— solo utilizan dos de ellos.
Los motivos son que los desarrolladores de sistemas no encuentran que tenga ninguna ventaja realmente significativa y que complica portar el sistema operativo a procesadores que solo soportan dos modos.
