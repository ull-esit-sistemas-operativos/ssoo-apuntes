...
Antiguamente sé denominada Win32 API, pero Microsoft ha querido aglutinar bajo una misma denominación las distintas versiones de la API de Windows que han existido, como Win16 —usada en las versiones de 16 bits de Windows— o Win64 —que es la variante de Win32 adaptada a arquitecturas de 64 bits—.
...
* *Mediante la pila del proceso* se insertan los parámetros de la llamada al sistema en la pila del proceso —que se suele usar para guardar variables locales y, en algunas arquitecturas, los argumentos pasados al llamar funciones— y el sistema operativo los recuperá de allí durante la llamada al sistema.
...
+
Es utilizando, por ejemplo, en sistemas BSD y en Windows XP y posteriores para x86 de 32 bits.

[[linux_mips_syscall]]
.Llamada al sistema en Linux MIPS.
====
Vemos como invocar directamente la llamada al sistema {linux_write} en Linux para MIPS.
...
Aunque veamos que es una única instrucción, lo que realmente va a ocurrir es que el sistema operativo va a tomar el control de la CPU para realizar la tarea solicitada.
...
El problema es que como se hacen mediante instrucciones en lenguaje ensamblador (véase el <<linux_mips_syscall>>) no son demasiado cómodas utilizar.
...
* Es parte del sistema operativo, por lo que se distribuyen con él.
...
Lenguajes distintos de C o {cpp} pueden tener difícil usar las funciones de la librería del sistema.
...
Incluso en C o {cpp} puede ser interesante tener acceso a funcionalidades adicionales a las ofrecidas por la API del sistema operativo: estructuras de datos, algoritmos de ordenamiento o búsqueda, funciones para manipular cadenas, funciones matemáticas, etc.
...
Por ejemplo, utilizando clases y objetos en lenguajes que soportar programación orientada a objetos.

====
Por eso, junto a cada intérprete o compilador de un lenguaje de programación suele ir una *librería estándar* que ofrece clases o funciones con las que los programas pueden acceder a los servicios del sistema operativo y realizar las tareas más comunes de forma más sencilla.
...
La *librería estándar* necesitan acceder a los servicios del sistema operativo para, a su vez, dar servicio a los programas que la usan.
...
Para ello la librería estándar utiliza la _librería del sistema_ que acompaña al sistema operativo.
====
...
Si embargo en C, {cpp} y otros lenguajes, todo lo que son flujos de datos se generalizan en el concepto de flujo o _stream_ (véase {clang_stiod} e {cpp_iostream}).
...
Sin embargo, debemos valar que así perdemos portabilidad, ya que ahora nuestro programa ya no podrá usarse allí donde haya un compilador o intérprete de nuestro lenguajes, sino solo en sistemas operativos con una *librería del sistema* compatible.
****
...
<7> Archivo abierto del que copiar los atributos para copiarlo en este,
en caso de crearlo.
...
En la <<api_posix>> se puede observar un ejemplo similar en GNU/Linux —un sistema compatible POSIX— pero en esta ocasión con programas en C y {cpp}.
