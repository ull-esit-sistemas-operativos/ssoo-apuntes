...
Antiguamente se denominaba Win32 API, pero Microsoft ha querido aglutinar bajo una misma denominación las distintas versiones de la API de Windows que han existido, como Win16 —usada en las versiones de 16bits de Windows— o Win64 —que es la variante de Win32 adaptada a arquitecturas de 64bits—.
...
Por ejemplo, en Linux para x86 la llamada al sistema {linux_open} —que se utiliza para abrir archivos— se identifica con el número 2 o con el 5, según si es en 64 o en 32bits, respectivamente.
...
También en Linux para x86 32bits, cuando el número de parámetros es superior a 6.
...
+
Es utilizado, por ejemplo, en sistemas UNIX BSD y en Windows XP y posteriores para x86 de 32bits.
...
En {syscall_s} se puede ver un ejemplo completo similar, pero para Linux x86 de 64bits.
====
