...
* La familia derivada de *BSD*, en la que se incluyen sistemas libres como: {freebsd}, https://es.wikipedia.org/wiki/NetBSD[NetBSD], https://es.wikipedia.org/wiki/OpenBSD[OpenBSD], {darwin} y https://es.wikipedia.org/wiki/DragonFly_BSD[DragonFly BSD], entre muchos otros.
...
La primera versión —Windows NT 3.1— lanzada el 13 de julio de 1993, era un sistema operativo _microkernel_ (véase el <<_mach>> un poco más adelante) multiplataforma que corría sobre procesadores {x86}, https://es.wikipedia.org/wiki/DEC_Alpha[DEC Alpha], https://es.wikipedia.org/wiki/MIPS_(procesador)[MIPS R4000] y https://es.wikipedia.org/wiki/PowerPC[PowerPC].
...
Por tanto, la mezcla de Mach con BSD de OpenStep es la base del sistema operativo {macos} actual de Apple.
