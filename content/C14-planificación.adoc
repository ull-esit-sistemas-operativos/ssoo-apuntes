[[planificación_de_la_cpu]]
= Planificación de la CPU
include::../config/attributes.adoc[]
:chapdir: C14-planificación
...
* Puesto que un proceso puede ser expropiado en cualquier momento —sin que pueda hacer nada para evitarlo— el sistema operativo debe proporcionar _mecanismos de sincronización_ (véase el <<sincronización>>) para coordinar el acceso a datos compartidos que podrían estar siendo modificados por el proceso que abandona la CPU y que puede necesitar el que entra en ella.
...
Por ello es interesante disponer de algún criterio para poder comparar los algoritmos y determinar cuál es el mejor.
...
Tiempo de respuesta(((tiempo, respuesta))):: Es el intervalo de tiempo que transcurre desde que se lanza un evento —se pulsa una tecla, se hace clic con el ratón o llega un paquete por la interfaz de red— hasta que se produce la primera respuesta del proceso.
...
Esta selección de criterios no solo es adecuada para los sistemas interactivos, ya que existen muchos otros casos donde es interesante seleccionar un planificador de la CPU que minimice el tiempo de respuesta.
...
image::{chapdir}/histogramas_tiempo_de_ráfagas.svg[]

La curva que relaciona la frecuencia de las ráfagas de CPU con la duración de las mismas tiende a ser exponencial o hiperexponencial (véase la <<fig-ráfagas-de-cpu>>) aunque varía enormemente entre tipos de tareas y sistemas informáticos distintos.
...
Supongamos que 4 procesos llegan a la *cola de preparados* en los tiempos indicados en la <<tabla-problema-fcfs>>.
...
Aunque el tiempo total necesario para ejecutar las ráfagas de los 4 procesos, los criterios utilizados reflejan que el algoritmo se comporta mucho mejor en el segundo caso.
...
Para entenderlo, analicemos lo que está pasando en el ejemplo de la <<tabla-problema-fcfs>>:

. Al proceso P1 se le asigna la CPU.
...
Por tanto, mientras los procesos esperan para utilizar la CPU, los dispositivos de E/S permanecen desocupados.
...
Con el tiempo el resto de procesos terminarán sus operaciones y, nuevamente, tienen que esperar en la *cola de preparados* a que el proceso P1 termine de utilizarla.

Esto nos permite llegar a la conclusión de que en cierto orden de llegada la mayor parte de los procesos esperan constantemente detrás de uno para poder realizar su trabajo.
Esto reduce la utilización de la CPU y de los dispositivos de E/S por debajo de lo que sería posible, si los procesos más cortos se ejecutasen primero.
...
Por eso el algoritmo SJF se utiliza frecuentemente como planificador de la *cola de trabajos*, donde se puede obligar al usuario a especificar un tiempo de ejecución máximo, al enviar el trabajo a dicha cola.
...
* stem:[tau_(n+1)] es la estimación de tiempo de la siguiente ráfaga de CPU
* stem:[t_n] es el tiempo real de la última ráfaga
* stem:[alpha] es el peso relativo del tiempo real de la última ráfaga.
* stem:[tau_(n)] es la estimación de tiempo de la última ráfaga.

La expresión es recursiva, dado que stem:[tau_n] se calcula usando la ecuación con stem:[t_(n-1)] y stem:[tau_(n-1)]; que a su vez depende de stem:[t_(n-2)] y stem:[tau_(n-2)], y así sucesivamente.

Si desarrollamos la fórmula sustituyendo los valores, veremos que stem:[t_n] se pondera con stem:[alpha], stem:[t_(n-1)] con stem:[(1 - alpha)alpha], stem:[t_(n-2)] con stem:[(1 - alpha)^2alpha], y así sucesivamente:
...
Si stem:[alpha] = 1, stem:[tau_(n+1)] = stem:[alpha*t_n], ignorando el resto del histórico.
En otro caso, dado que tanto stem:[alpha] como stem:[1 - alpha] son menores de 1, cada término sucesivo tiene menor peso que su predecesor, haciendo que los stem:[t_n] contribuyan menos cuanto más alejados del tiempo actual.
...
En *SRTF* se compara el tiempo de la siguiente ráfaga de CPU del nuevo proceso, con el tiempo de ráfaga que le queda al proceso en ejecución.
...
Se espera que el proceso que actualmente se está ejecutando termine su ráfaga de CPU voluntariamente.
...
Si las prioridades se asignan en relación con el tiempo de la próxima ráfaga de CPU, su comportamiento es el mismo que el del *SJF*; por lo que se considera a este último un caso particular de algoritmo de *planificación con prioridades*.

El algoritmo de planificación con prioridades puede ser *expropiativo* o *cooperativo*:

* En el caso *expropiativo*, cuando un proceso llega a la *cola de preparados* su prioridad es comparada con la del proceso en ejecución.
...
Supongamos que 5 procesos llegan a la cola de preparados en los tiempos indicados en la <<tabla-problema-prioridad>>.
...
image::{chapdir}/prioridad_cooperativo.svg[]
...
Por el contrario, un criterio como el tiempo de ráfaga de CPU puede dar lugar a una prioridad variable, que se ajusta cada vez que se tiene una estimación mejor.

==== Muerte por inanición

El mayor problema de este tipo de planificación es el *bloqueo indefinido*(((bloqueo, indefinido))) o *((muerte por inanición))*.
...
El algoritmo *((RR))* (_Round-Robin_) es similar al *FCFS*, pero añadiendo la expropiación para conmutar entre procesos cuando llevan cierta cantidad de tiempo ejecutándose en la CPU.
...
Este hará el cambio de contexto para asignar la CPU al siguiente proceso y el que abandona la CPU es insertado al final de la *cola de preparados*.
...
Sin embargo el número de cambios de contexto será mayor, por lo que la ejecución de los procesos será más lenta.
+
...
Estos tiempos son mucho mayores que los tiempos de cambios de contexto, que generalmente son inferiores a 10 µs.
====
...
Por ejemplo, supongamos que 4 procesos con diferentes ráfagas de CPU llegan juntos y compiten por el uso de la CPU:
...
Tomando un periodo de 4 ms., en el primer periodo a cada proceso le corresponde 1 ms. porque hay 4 procesos en estado *preparado*.
...
Finalmente, al terminar el periodo 6, acaba el proceso P1 —que ya debe haberse ejecutado 8 ms.— por lo que P4 puede usar los 4 ms. del periodo 7 y terminar ráfaga.

En la siguiente tabla se ilustra este reparto indicando el tiempo de cada cuanto que corresponde a cada proceso:

[[tbl-planificación-equitativa-ideal]]
...
Sin embargo, debemos tener en cuenta que en un instante dado, no puede ejecutarse más de un proceso en la CPU.
Es decir, durante todo el periodo 1, realmente no pueden ejecutarse los 4 procesos al mismo tiempo.
El planificador tiene asignar la CPU en algún orden, de forma que el tiempo finalmente usado por cada proceso en cada periodo sea aproximadamente el calculado en la tabla anterior.

En la siguiente figura puede verse un posible reparto:

image::{chapdir}/equitativo1.svg[]

El algoritmo de *planificación equitativa* es muy similar al algoritmo *RR*, pero en la *planificación equitativa* la ventana de tiempo que finalmente se asigna a cada proceso se calcula dinámicamente.

==== Implementación básica

Las implementaciones del algoritmo de *planificación equitativa* suelen utilizar el concepto de _tiempo virtual_ para seguir la pista de cuánto tiempo debe ejecutarse el proceso.
Por ejemplo, en el planificador de {zircon}<<Fuchsia-ZirconFairScheduler>> —el _microkernel_ de {google_fuchsia}— cuando un proceso stem:[P_i] entra en la *cola de preparados*, en el PCB de dicho proceso se actualizan dos tiempos:

* *Instante inicial stem:[t_(si)]* de la ráfaga de CPU del proceso, en el _tiempo virtual_ de la CPU.

* *Instante final stem:[t_(fi)]* de la ráfaga de CPU del proceso, en el _tiempo virtual_ de la CPU.

Estos tiempos se dicen que están en _tiempo virtual_ de la CPU para señalar que idealmente el proceso comenzaría su ejecución en stem:[t_(si)] y terminaría en stem:[t_(fi)], aunque esto difícilmente va a ocurrir en la realidad porque el tiempo de CPU hay que repartirlo entre los diferentes procesos.

[NOTE]
{zircon} es un _microkernel_ multihilo con modelo uno a uno, por lo que su planificador trabaja con hilos.
...
Si stem:[t] es el instante de tiempo en el que un proceso stem:[P_i] entra en la cola de preparados, el instante inicial stem:[t_(si)] y el instante final stem:[t_(fi)] se calculan de la siguiente manera:
...
donde stem:[M] es la porción mínima de tiempo de CPU real que se puede asignar a cada proceso y stem:[g] es la cantidad de estas porciones de tiempo que se quiere repartir entre todos los procesos que están en disposición de ser ejecutados.
Es decir, en el ejemplo anterior stem:[gM = 4" ms"], por lo que si la porción mínima fuera stem:[M = 1" ms."], cada periodo tendría 4 porciones de tiempo para repartir.

En la *cola de preparados* los procesos son ordenados por el valor de stem:[t_(fi)], de forma que primero se planifican los procesos que tiene ráfagas que virtualmente terminarían antes.

El planificador de {zircon} tiene una serie de parámetros ajustables:

* La *granularidad mínima stem:[M]* que, como hemos comentado, es la porción mínima de tiempo de CPU real que se puede asignar a cualquier proceso.

* La *latencia objetivo stem:[L]*, que es el periodo de tiempo de CPU mínimo que se va a repartir entre los procesos.
Es decir, dentro de un periodo de *latencia objetivo* stem:[L] hay stem:["floor"(L / M)] porciones de tiempo para repartir entre los procesos.

Si hay stem:[n] procesos que se pueden ejecutar, lo que hace el planificador es calcular stem:[g] de la siguiente manera:
...
de forma que siempre haya suficientes porciones stem:[M] para que cada proceso pueda tener al menos una, limitando por debajo el número de porciones a repartir a stem:["floor"(L / M)].

Cuando se escoge un proceso para su ejecución, se calcula el cuanto real stem:[t_i] durante el que se le asignará la CPU:
...
de forma que se reparten las stem:[g] porciones entre los stem:[n] procesos.

Si hay muchos procesos, stem:[g = n], por lo que a cada proceso se le asigna un tiempo de CPU de stem:[M].
Si hay pocos procesos, stem:[g = "floor"(L / M)], por lo que a cada proceso se le asignan varias porciones stem:[M] de tiempo de CPU del periodo stem:[L].

==== Planificación equitativa ponderada

(((planificación, de CPU, equitativa ponderada)))
Al igual que en los algoritmos anteriores de planificación, en ocasiones puede ser interesante priorizar unos procesos frente a otros, tanto por motivos ajenos al sistema operativo como por motivos internos.
...
La *planificación equitativa* resuelve este problema permitiendo que a los procesos se les asignen pesos y repartiendo más tiempo de CPU a los procesos con mayor peso.
A esta generalización del planificador equitativo se la conoce como *planificador equitativo ponderado*.

Linux, desde la versión 2.6.23, utiliza un tipo de *planificador equitativo ponderado* denominado *((CFS))* (_Completely Fair Scheduler_) o *planificador completamente equitativo*(((planificador, completamente equitativo)))<<Jones2018>>.
Otro ejemplo es {zircon} que, como hemos comentado, está inmerso en cambiar su actual planificador basado en colas multinivel con prioridades a una implementación del *planificador equitativo* que, de hecho, es realmente un *planificador equitativo ponderado*<<Fuchsia-ZirconFairScheduler>>.

==== Implementación de la planificación equitativa ponderada

En {zircon} cada proceso stem:[P_i] tiene un peso stem:[w_i] que indica la importancia del proceso en el reparto del tiempo de CPU.

El primer cambio —respecto a la implementación vista anteriormente— es que el instante final stem:[t_(fi)] realmente se calcula de la siguiente manera:
...
de forma que es los procesos con mayor peso stem:[w_i] tiene un stem:[t_(fi)] más pequeño, por lo que se colocan antes en la *cola de preparados*.

Cuando se escoge un proceso para su ejecución, se calcula el cuanto real stem:[t_(i)] durante el que se le asignará la CPU, asignándole un tiempo proporcional al peso del proceso stem:[w_i] respecto al peso del resto de los procesos con los que compite:
...
donde stem:[W] es la suma de todos los pesos stem:[w_i] para los stem:[n] procesos preparados para ser ejecutados.

==== Ejemplo de planificación equitativa ponderada

Ilustremos el algoritmo con un ejemplo con stem:[L=5] y stem:[M=1]:
...
image::{chapdir}/equitativo2.svg[]

Al empezar, el primer proceso está solo cuando se le asigna la CPU, por lo que puede consumir toda la latencia de planificación stem:[L] en el primer periodo.
Posteriormente, cuando vuelve a la *cola de preparados* en el instante 5, han llegado el resto de procesos, por lo que tiene que competir con ellos por la CPU.

A partir de ese instante el orden para asignar la CPU viene determinado por stem:[t_(fi)], cuya expresión depende de stem:[g].
En este caso stem:[g=M/L=5] en todo momento, puesto que hay menos procesos que porciones de tiempo stem:[M] en stem:[L].

En la siguiente tabla se puede observar los valores de stem:[t_(si)] y stem:[t_(fi)] para cada proceso en el instante 5:
...
Aunque el proceso stem:[P_4] es el último en llegar, es el siguiente en ser planificado, porque al tener un peso más alto su stem:[t_f4] es el más bajo.
Entonces se calcula el tiempo de cuanto stem:[t_4], que da 3, por lo que se planifica en la CPU durante ese tiempo.

Sin embargo, stem:[P_4] no consume su tiempo de cuanto porque su ráfaga terminar antes, por lo que en el instante 7 se tiene que planificar el siguiente proceso en la *cola de preparados*.
En este caso sería stem:[P_3], con un tiempo de cuanto de 2.

Para calcular stem:[t_3] es importante recordar que stem:[W = 5], puesto que stem:[P_4] ya no está en la *cola de preparados* por lo que su peso no se incluye.

En el instante 9, el proceso stem:[P_3] es expulsado de la CPU y vuelve a la *cola de preparados*.
Entonces, se tienen que actualizar sus valores para stem:[t_(si)] y stem:[t_(fi)] con el objeto de determinar cuál será el próximo proceso planificado:
...
Por tanto, el siguiente proceso a planificar es stem:[P_2] con stem:[t_2 = 1].
...
|P1
|16
>|[.lightcell]#(0-0) + (10-5) + (13-12)=#|6
>|[.lightcell]#(16-0)=#|16

|P2
|17
>|[.lightcell]#(9-1) + (16-10)=#|14
>|[.lightcell]#(17-1)=#|16

|P3
|14
>|[.lightcell]#(7-2) + (12-9)=#|7
>|[.lightcell]#(13-2)=#|11
...
Los procesos son asignados permanentemente a alguna de dichas colas, cada una de las cuales puede tener un algoritmo de planificación distinto.

La asignación de un proceso a una cola se hace en relación a alguna una característica del proceso.
...
Por ejemplo, si los procesos se clasifican en interactivos o no interactivos, los primeros pueden ir a una cola con planificación *RR* mientras los segundos van a una con *FCFS*.

[[fig-colas-multinivel]]
.Ejemplo de planificación con colas multinivel.
image::{chapdir}/planificación_colas_multinivel.svg[]

Una cuestión interesante es cómo seleccionar la cola que debe escoger al siguiente proceso a ejecutar:

* Una opción común en los sistemas actuales es utilizar un *planificador con prioridades*.
...
+
Por ejemplo, en la <<fig-colas-multinivel>>, mientras un proceso de prioridad 1 esté preparado, no se escoge ningún otro de prioridad inferior.
...
La *planificación de colas multinivel* con un *planificador con prioridades* para escoger la cola adecuada, es con diferencia la opción más común en los sistemas operativos modernos.
...
[[fig-planificación-colas-multinivel-realimentadas]]
.Ejemplo de planificación con colas multinivel realimentadas.
image::{chapdir}/planificación_colas_multinivel_realimentadas.svg[]
...
En las colas se usa el algoritmo *RR* para seleccionar el siguiente proceso, siendo el *cuanto* de la cola mayor cuanto menos prioritaria es la cola (véase la <<fig-planificación-colas-multinivel-realimentadas>>).
...
Con este algoritmo los procesos limitados por E/S suelen ejecutarse la mayor parte del tiempo con prioridades más altas que los limitados por CPU.
Por ejemplo, usando los valores del esquema de la <<fig-planificación-colas-multinivel-realimentadas>>, los procesos de ráfagas de CPU entre 20 y 80 ms acaban cayendo a la cola de prioridad 1 tras 20 ms de ejecución.
Así dejan paso a los procesos con ráfagas menores de 20 ms, que siempre se ejecutan con prioridad 0.
Finalmente, los procesos de ráfagas mayores de 80 ms van a la cola FCFS, desde donde solo tendrán acceso a la CPU cuando no haya ningún proceso de los otros tipos en la *cola de preparados*
...
[[fig-planificación-rr-virtual]]
.Ejemplo de planificación con RR virtual.
image::{chapdir}/vrr.svg[]

Tal y como se ilustra en la <<fig-planificación-rr-virtual>>, en el *RR virtual* los procesos por lo general tienen prioridad 1.
...
Esta solución puede llevar a que si hay muchos procesos limitados por la E/S, estos acaparen la CPU y no den oportunidad de ejecutarse a los procesos en la cola de prioridad 1.
...
Desde el punto de vista de Windows API, todo proceso pertenece a alguna de las 6 clases de prioridad de la <<tabla-win32-clases-prioridad>><<Win32-SchedulingPriorities>>.
...
image::{chapdir}/administrador_de_tareas.jpg[]

Con el *Administrador de tareas* de Windows podemos alterar fácilmente la clase de prioridad de un proceso durante su ejecución (véase la <<fig-windows-task-manager>>).

[[tabla-win32-prioridad-hilos]]
.Clases de prioridad en Windows API.
[cols="2*^" options=header]
|====
|Prioridad
...
Al mismo tiempo, cada hilo del sistema tiene alguno de las prioridades de la <<tabla-win32-prioridad-hilos>>.
La prioridad de un hilo recién creado es `THREAD_PRIORITY_NORMAL`, pero se puede cambiar usando la función {win32_setthreadpriority}.
...
[[tabla-win32-prioridad-estática]]
...
Cuando llega un hilo a la cola de preparados, expropia la CPU al hilo que la tiene asignada si este tiene menor prioridad.
...
[NOTE]
====
Desde el Intel Pentium las CPU de la familia x86 incorporan un contador de marca de tiempo (_Time Stamp Counter_ o TSC) de 64 bits que indica el número de ciclos transcurridos desde el último reinicio del procesador<<Wikipedia-TSC>>.
====
...
También se bonifica cualquier hilo que gestione elementos de la interfaz gráfica cuando despierta para responder a eventos del sistema de ventanas.

Para evitar la *muerte por inanición*, el planificador escoge cada segundo unos pocos hilos que llevan esperando aproximadamente 4 segundos, les triplica el *cuanto* y les aumenta la prioridad a 15.
...
Generalmente las tareas son entregadas al sistema operativo junto con una declaración de las restricciones de tiempo —periodicidad y límite de tiempo— y la cantidad de tiempo que necesitan para ejecutarse.
El planificador solo admitirá las tareas si puede garantizar el cumplimiento de las restricciones de tiempo, rechazándolas en caso contrario.
...
Esto puede generar excesos en la cantidad de recursos asignados a los procesos de tiempo real, así como inanición y grandes retrasos en la ejecución del resto de los procesos, pero es compatible con los sistemas de propósito general.

Además nos permite conseguir sistemas de propósito general que soporten multimedia, videojuegos y otras tareas que no funcionan de manera aceptable en un entorno que no implementara tiempo real flexible.
...
// TODO:  Dibujo para explicar el problema
...
Esto aumenta la *latencia de asignación*, dado que algunas llamadas al sistema pueden ser muy complejas y requerir mucho tiempo para completarse.
...
Debido a la función que realizan los puntos de expropiación, solo pueden ser colocados en lugares seguros del código del núcleo.
...
Para resolverlo se utiliza un *((protocolo de herencia de la prioridad))*, donde un proceso de baja prioridad hereda la prioridad del proceso de más alta prioridad que espera por un recurso al que el primero está accediendo.
...
(((multiprocesamiento, simétrico)))
Cuando utilizamos *multiprocesamiento simétrico* o *SMP*, cada procesador ejecuta su propia copia del núcleo del sistema operativo y se autoplanifica mediante su propio planificador de CPU.
