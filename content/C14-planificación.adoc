...
==== Planificación equitativa podenrada
...
La suma _N_ de procesos en estado *preparado* y *ejecutando* en un instante dado es el número total de procesos que compiten por la CPU.

Cada proceso _i_ tiene una serie de propiedades:
...
Sea _t_ es el instante de tiempo en el que un proceso _i_ entra en la cola de preparados, en relación al tiempo virtual el proceso comienza a ejecutarse en la CPU desde ese mismo instante y necesitará más tiempo para terminar cuanto mayor sea su peso.
...
Por lo tanto, el proceso _i_ no entrará en la CPU nada más llegar en _t_, pero si se colocará en la cola de preparados en orden ascendente en relación al instante final _T~fi~_.
Es decir, los procesos con mayor peso _w~i~_ se colocarán antes en la cola de preparados, con bastante probabilidad.

Cuando un proceso _i_ es seleccionado para pasar al estado *ejecutando*, se calcula su cuanto de tiempo.
...
donde _W_ es la suma de los pesos de todos los procesos en competición por la CPU.
Cuando un proceso entra en la cola de preparados su peso se suma a _W_, mientras que cuando pasa al estado *esperando* su peso se resta.

Finalmente, podemos calcular el cuanto del proceso _i_: 
