...
* _{tau}_~_n_+1~ es la estimación de tiempo de la siguiente ráfaga de CPU
* _t~n~_ es el tiempo real de la última ráfaga
* _{alpha}_ es el peso relativo del tiempo real de la última ráfaga.
* _{tau}~n~_ es la estimación de tiempo de la última ráfaga.

La expresión es recursiva, dado que _{tau}~n~_ se calcula usando la ecuación con _t_~_n_-1~ y _{tau}_~_n_-1~; que a su vez depende de _t_~_n_-2~ y _{tau}_~_n_-2~, y así sucesivamente.

Si desarrollamos la fórmula sustituyendo los valores, veremos que _t~n~_ se pondera con _{alpha}_, _t_~_n_-1~ con (1 - _{alpha}_)_{alpha}_, _t_~_n_-2~ con(1 - _{alpha}_)^2^_{alpha}_, y así sucesivamente:
...
Si _{alpha}_ = 1, _{tau}_~_n_+1~ = _{alpha}t~n~_, ignorando el resto del histórico.
En otro caso, dado que tanto _{alpha}_ como 1 - _{alpha}_ son menores de 1, cada término sucesivo tiene menor peso que su predecesor, haciendo que los _t~n~_ contribuyan menos cuanto más alejados del tiempo actual.
...
Es decir, en el ejemplo anterior stem:[gM = 4\text( ms.)], por lo que si la porción mínima fuera stem:[M = 1\text( ms.)], cada periodo tendría 4 porciones de tiempo para repartir.
...
Es decir, dentro de un periodo de *latencia objetivo* stem:[L] hay stem:[\text{floor}(L / M)] porciones de tiempo para repartir entre los procesos.
...
de forma que siempre haya suficientes porciones para que cada proceso pueda tener al menos una, limitando por debajo el número de porciones a repartir a stem:[\text{floor}(L / M)].

Cuando se escoge un proceso para su ejecución, se calcula el cuanto real stem:[t_{i}] durante el que se le asignará la CPU:
...
Aunque el proceso stem:[P_4] es el último en llegar, es el siguiente en ser planificado, porque al tener un peso más alto su stem:[t_{f4}] es el más bajo.
...
(((sistema, tiempo real, estricto)))(((tiempo real, estricto)))(((hard real-time)))
...
(((sistema, tiempo real, flexible)))(((tiempo real, flexible)))(((soft real-time)))
...
(((sistema, multiprocesamiento, asimétrico)))(((multiprocesamiento, asimétrico)))
...
(((sistema, multiprocesamiento, simétrico)))(((multiprocesamiento, simétrico)))
