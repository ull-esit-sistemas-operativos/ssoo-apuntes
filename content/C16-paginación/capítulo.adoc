...
Si las direcciones virtuales son de 32bits, eso deja para el número de página stem:[p]:
...
En un sistema de 32bits con páginas de 4 KiB —como del que hablamos antes— el espacio de direcciones virtual tiene 1{K}048{K}576 páginas.
...
Utilizaba un espacio de direcciones de 16bits y un tamaño de *páginas* de 8 KiB, por lo que sólo necesitaba 8 registros dedicados para alojar toda la *tabla de páginas*.
...
Por ejemplo, en un sistema de 32bits, muy pocos procesos necesitan los 3 GiB disponibles como máximo para cada proceso —el 1 GiB restante suele estar ocupado por el núcleo del sistema—.
...
Así, en un sistema de 32bits con páginas de 4 KiB y 4 bytes por entrada, se necesitan 4 MiB de memoria para almacenar la *tabla de páginas*.
...
Por ejemplo, supongamos un sistema con un espacio de direcciones virtual de 32bits:
...
Por ejemplo, si asumimos el caso anterior de un sistema con un espacio de direcciones de 32bits y un tamaño de
...
Estos sistemas utilizaban un arquitectura de 32bits con un tamaño de página de 512 bytes.
...
Los 2bits de orden más alto stem:[s] de las direcciones virtuales se utilizaban para indicar la *sección*.
Cada *sección* estaba dividida en *páginas* de 512 bytes, por lo que los siguientes 21bits de las direcciones virtuales stem:[p] eran utilizadas para seleccionar la *página* concreta.
...
Cada *página* es de 4 KiB —como en el resto de la familia x86— pero como cada entrada en la *tabla de páginas* es de 8 bytes —con el fin de poder almacenar direcciones de 64bits— en cada una caben 512 entradas, por lo que los *números de página* de cada nivel necesitan 9bits.
Eso significa que de las direcciones virtuales se utilizan actualmente 48bits —resultado de multiplicar 4 niveles por 9bits cada uno más 12bits de desplazamiento—, aunque el limite de la arquitectura para las direcciones virtuales sea de 64bits.
