...
. El *número de página*(((número, página))) es utilizado por la MMU para indexar la *tabla de páginas*(((tabla, páginas))), que contiene el *número de marco*(((número, marco))) stem:[f] de cada *página* en la memoria física.
...
image::dirección_virtual_paginación.svg[]
...
por lo que el espacio de direcciones virtual tiene stem:[2^20] páginas —es decir, 1.048.576 páginas—.
...
La otra opción es alojar *tabla de páginas* del proceso actual en la memoria, normalmente en un formato definido por la CPU.
...
Además, esto tiene la ventaja de que el *cambio de contexto* sea más rápido —respecto al uso de registros para almacenar la tabla de páginas— puesto que sólo es necesario cargar un único registro más —el *PTBR*— durante el mismo.

=== TLB

La solución al retraso originado por el acceso a la tabla de páginas, cuando ésta está en la
memoria, pasa por que el sistema disponga de un pequeño caché de traducciones en hardware llamado *((TLB))* (_Translation Look-aside Buffer_).
...
Los algunos algoritmos de reemplazo utilizados van desde elegir una aleatoriamente hasta el *((LRU))* (_Least Recently Used_).
...
Sin embargo, un proceso no tiene porque utilizar todas las entradas de la *TLB*.
Por lo que sería mas interesante, no tener que borrar las entradas de procesos anteriores, mientras no sean necesarias, por si estos vuelven a ser ejecutados en la CPU.
...
En éste tipo de *TLB*, en la *clave* se buscan pares *(número de página, ASID)*, donde el primero proviene de la dirección virtual y el segundo es el *ASID* del proceso actual.
...
Los *bits de protección* de protección pueden ser:

* *Solo lectura*.

* *Lectura -- Escritura*.
En algunos sistemas hay un bit específico para este permiso, mientras que en otros se utilizan bit separados, como: *lectura*, *escritura* y *ejecución*; que se pueden combinar libremente.
...
Si esto no lo es, se genera una excepción de violación de protección de memoria, dado que el acceso en un modo no autorizado se considera una instrucciones privilegiada.
...
* Cuando la *página no es inválida*, la página no existe en el espacio de direcciones virtual del proceso.
...
Generalmente, porque no se le ha asignado un *marco* de memoria física, porque esa página no está siendo utilizada por el proceso.

[[bit_de_válido_en_la_tabla_de_páginas]]
.Bit de válido en la tabla de páginas.
image::bit_de_válido_en_la_tabla_de_páginas.svg[]
...
Mientras que todas las *paginas* en direcciones por encima de la 5119 están marcadas como ilegales.
...
Como un proceso suele ocupar muy poco de su espacio de direcciones virtual, suele ser un desperdicio de memoria crear y almacenar un *tabla de páginas* completa, con una entrada para cada *página* del espacio de direcciones.

Para evítalo, en algunas CPU existe el registro *((PTLR))* (_Page-Table Length Register_) que se utiliza para indicar el tamaño actual de la *tabla de páginas*.
...
|*número de páginas*
|= stem:[2^32 / 2^12] = stem:[2^{32-12}] = stem:[2^20] = 1.048.576 entradas
...
Una forma de resolver este problema es partir la *tabla de páginas*, de manera que no sea necesario asignarle la memoria de forma contigua.
...
image::paginación_jerárquica.svg[]

Por ejemplo, si asumimos el caso anterior de un sistema con un espacio de dirección de 32 bits y un tamaño de
página de 4 KiB, entonces podemos dividir la tabla de páginas de 1.048.576 entradas —4 MiB si cada entrada necesita 4 bytes— en 1024 porciones, cada una de las cuales cabría en un *marco* de 4 KiB.

Estos *marcos*, a su vez, pueden ser mapeadas por 1024 entradas con las direcciones físicas de cada *marco*.
...
Sin embargo, ahora la *tabla de páginas* está dividida en *marcos*, que no tienen porque ser asignados de forma contigua en la memoria.
...
image::dirección_virtual_paginación_jerárquica.svg[]
...
image::dirección_virtual_vax.svg[]
...
En general, en la *paginación jerárquica* de stem:[N] niveles el *número de página* stem:[p] de cada dirección virtual es dividido en stem:[N] números: stem:[{ p_1, p_2, p_3, \ldots, p_N}], donde:

. stem:[p_1] se utiliza para indexar la *tabla de páginas externa*(((tabla, paginas, externa))) —también llamada *directorio de páginas*(((directorio, páginas))) o *tabla de páginas de nivel 0*(((tabla, paginas, nivel 0)))— cuya dirección conoce la CPU mediante el *PTBR*.
...
Debido a que la traducción funciona desde las *tablas de páginas* de nivel superior —nivel 0— hacia las de nivel inferior —nivel stem:[N-1]— a esta estructura también se la conoce como *tabla de paginas directa*(((tabla, páginas, directa))).

[NOTE]
====
Los procesadores {decalpha} y {mips} utilizan una variante denominada *tabla de páginas virtualizada*(((tabla, paginas, virtualizada))).
...
Así, si la consulta a la *TLB* falla, se indexa la *tabla de página* directamente con direcciones virtuales usando el *número de página* completo.
