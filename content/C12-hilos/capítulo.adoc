...
Si queremos hacer varias tareas al mismo tiempo, estamos obligados a crear varios procesos y seleccionar un mecanismo de comunicación para estos colaboren.
...
// Más parecido al de la memoria del procesos en el capítulo 9.

[[multihilo]]
.Comparación entre procesos monohilo y proceso multihilo.
image::procesos_multihilo.svg[]

====
Cada hilo tiene una serie de recursos propios dentro del proceso (véase la <<multihilo>>):

* *El identificador del hilo*, que es único para cada hilo y sirve para identificarlos, de la misma manera que lo hace el identificador de proceso con cada proceso.

* *El contador de programa*, que es el registro de la CPU que indica la dirección de la próxima instrucción del hilo que debe ser ejecutada por la CPU.
...
* *La pila*, que contiene datos temporales como argumentos y direcciones de retorno de las funciones y variables locales.
...
Sin embargo hay otros recursos del procesos que se comparten entre todos los hilos del mismo (véase la <<multihilo>>):
...
Eso quiere decir, por ejemplo, que cualquier hilo puede accede y modificar una variable global o una asignada dinámicamente mediante {clang_malloc} o {cpp_new}.
...
Una aplicación multihilo interactiva puede continuar ejecutando tareas aunque uno o varios hilos de la misma estén bloqueados o realizando operaciones muy lenta, mejorando así el tiempo de respuesta al usuario.
...
Mientras que para hacer lo mismo en un navegador monohilo habría que utilizar comunicaciones asíncronas.
De lo contrario, mientras el proceso está en estado *esperando*, a la espera de que lleguen los datos a través de la red, no puede atender las acciones del usuario.
...
Por eso los sistemas operativos modernos ha desarrollado diversas técnicas para que sea lo más eficaz posible.
...
Por ejemplo, en Microsoft Windows crear un procesos puede ser 300 veces más costoso que un hilo. Mientras que en sistemas Linux es 3 veces más lento, por la eficiencia de {linux_fork}.
...
Por el contrario, un proceso monohilo sólo se puede ejecutar en una CPU a la vez, independientemente de cuantas CPU estén disponibles para ejecutarlo.
...
Por ese motivo se los denomina *hilos de usuario*(((hilos, usuario))).

Como el código y los datos de la librería residen en el espacio de usuario, por lo que invocar una función de la misma se reduce a una simple llamada a una función, evitando el coste de hacer llamadas al sistema.
...
Por ese motivo se los denomina *hilos de núcleo*(((hilos, núcleo))).

Aparte del *PCB* que vimos en el <<bloque_de_control_de_proceso>>, cada hilo tiene una estructura llamada *bloque de control del hilo*(((bloque de control, hilo))) o ((TCB)) (_Thread Control Block_) que representa a cada hilo en el sistema operativo y que guarda información sobre su estado de actividad actual.

En estos sistemas es el hilo la unidad básica de uso de la CPU.
Son los hilos los que se mueven por los estados del <<diagrama_estado_proceso>> y las colas de la <<colas_de_planificación_procesos>> y no los procesos.
El planificador de la CPU selecciona un hilo para ejecutarse en la CPU de entre todos los que están en el estado *preparado* en el sistema y el *cambio de texto* asigna la CPU a un hilo distinto al que la tiene asignada en el momento actual.

Por tanto, es en *TCB* —y no en el *PCB*— donde se guarda la información privada del hilo necesaria para la gestión de lo estados y para el cambio de contexto, como: los valores de los registros de la CPU y el contador de programas, el estado o la información de planificación de la CPU; además de un puntero al *PCB* al que pertenece el hilo con el resto de la información privada del proceso.
...
En el modelo *muchos a uno* los hilos que ve el proceso se mapean en un única «entidad planificable en la CPU» del núcleo.

Este, por lo general, es el modelo utilizado cuando el núcleo no soportan múltiples hilos de ejecución.
En ese caso, la única entidad planificable en la CPU que conoce el núcleo es el proceso, la librería de hilos se implementa en el espacio de usuario —dentro del proceso— y los hilos que ve el proceso son hilos de usuario (véase la <<modelo_muchos_a_uno>>).

// TODO: Ajustar el dibujo a la explicación

[[modelo_muchos_a_uno]]
.Modelo muchos a uno.
image::modelo_muchos_a_uno.svg[]
...
* Como sólo un hilo puede ser asignado al proceso, los hilos de un mismo proceso no se pueden ejecutar en paralelo en sistemas multiprocesador.
...
En el modelo *muchos a uno* cada hilo que ve el proceso se mapean en una «entidad planificable en la CPU» diferente del núcleo.
...
* Crear un hilo para un proceso implicar crear ciertas estructuras de datos en el núcleo.
...
* Las gestión de los hilos se hace con una librería en el espacio de núcleo, lo que requiere que el proceso haga llamadas al sistema para gestionarlos.
...
[[modelo_muchos_a_muchos]]
.Modelo muchos a muchos.
image::modelo_muchos_a_muchos.svg[]

====
En el modelo *muchos a muchos* se mapean los hilos de usuario en un menor o igual número de hilos de núcleo del proceso (véase la <<modelo_muchos_a_muchos>>).
...
* Permite a otro hilo de usuario del mismo proceso ejecutarse cuando un hilo hace una llamada al sistema que debe bloquearse, puesto que si esto ocurre el correspondiente hilo de núcleo queda bloqueado.
Sin embargo, el resto de los hilos de usuario pueden seguir ejecutándose en los otros hilos de núcleo del proceso.
====

Este modelo se soportada en sistemas {freebsd} y versiones antiguas de {netbsd}, así como en UNIX comerciales, como: {solaris} 8 y anteriores, {irixnix, {hpux} y {tru64}.
También Microsoft Windows —a partir de Windows 7— soporta este modelo gracias a incorporar un mecanismo denominado https://docs.microsoft.com/en-us/windows/win32/procthread/user-mode-scheduling[planificacion en modo usuario].
...
Dicha comunicación tiene como objeto ajustar dinámicamente el número de hilos del núcleo para garantizar la máxima eficiencia.
...
Debido a la complejidad del mecanismo descrito anteriormente y a la dificultad de coordinar el planificador de la libraría de hilos con el de la CPU para obtener un rendimiento óptimo, sistemas como Linux y {solaris} —a partir de la versión 9— han optado finalmente por el modelo *uno a uno*.
...
Esta variación se denomina, en ocasiones, modelo de *dos niveles* (véase la <<modelo_de_dos_niveles>>).

// TODO: Ajustar el dibujo a la explicación

[[modelo_de_dos_niveles]]
.Modelo muchos a uno.
image::modelo_de_dos_niveles.svg[]
...
Excepto por la características especial del *hilo principal* de que su finalización significa la terminación del proceso, todos los hilos son iguales entre sí.

En el <<ejemplo_pthread>> se puede como se usa {pthread_create} en sistemas POSIX que implementan {pthreads} para crear varios hilos y esperar a que terminen con {pthread_join}.
...
El código fuente completo de este ejemplo está disponible en {pthread_cpp} y, además, permite indicar el número que queramos para calcularle el factorial.
En {threads_cpp} se puede estudiar un ejemplo equivalente pero usando {cpp_thread} de la librería estándar de C++, por lo que también compila en sistemas no POSIX, como Microsoft Windows.
...
Si el sistema implementa al librería de hilos en el núcleo, por lo general, cada hilo tiene un identificador único; pero {pthreads} no ofrece una forma de obtenerlo.
...
<5> Los códigos de error son los mismos que hasta hora veíamos en {linux_errno}.
...
<7> Todo hilo tiene una función principal que será dónde comience la ejecución del hilo.
...
<9> En este ejemplo definimos `factorial_thread_args` para pasar losa argumentos a los hilos y lo pasamos como `void *` a la función principal.
...
Este se indica en la sentencia `return` de la función principal del hilo o invocando {pthread_exit} para terminar.
...
La diferencia es que ahora el desarrollador conoce de antemano los puntos donde podría terminar el hilo, lo que da una oportunidad de introducirlos solo dónde sea seguro terminar.
...
Se pueden cambiar entre estado y tipo de cancelación en cualquier momento, según lo que encaje mejor con las características de las distintas parte del código.
...
Uno de los pocos casos con los que es compatible es en bucles 100% dedicados a ejecutar cálculos en al CPU, como el siguiente: 
...
La *cancelación asíncrona* no se debe usar si el código reserva memoria dinámicamente o solicita otros recurso del sistema operativo, porque el hilo podría terminar en cualquier momento sin liberarlos.
...
En la terminología de {pthreads} a estos puntos se los denomina *puntos de cancelación*(((punto, cancelación))) y la inmensa mayoría de las llamadas al sistema que puede poner el hilo en estado *esperando* lo son por si mismas.
Por ejemplo, {linux_open}, {linux_close}, {linux_read}, {linux_write}, {linux_recv}, {linux_send}, {linux_poll} y {linux_sleep}, entre muchas otrsa (véase la lista en la sección «_Cancellation points_» de la documentación de {pthreads}).
...
Si no es seguro permitir la cancelación de un hilo en la invocación de una de estas funciones en nuestro código, se puede usa {pthread_setcancelstate} para desactivar temporalmente el mecanismo de cancelación.
Por ejemplo, una llamada a {clang_printf} como ayuda para depurar, en medio de los pasos para modificar una estructura de datos —como una lista enlazada o una cola— introduce un *punto de cancelación* en lugar poco seguro; porque 
...
La *cancelación en diferido* también presenta retos desde el punto de visto de evitar las fugas de memoria y de otros recursos cuando un hilo es cancelado.
...
Este código y la forma en que maneja los errores funciones bien en programas monohilo, porque estamos seguro de que al salir de la función o se completaron todas las etapas o ninguna.
...
Si `conn_open()` es llamada desde un hilo y ese hilo es cancelado, el hilo podría terminar a mitad de la función, sin liberar `handler`, creando un *fuga de memoria* que no se liberará hasta que el proceso termine.
Si `conn_open()` es llamada en multiples ocasiones, cada una es una oportunidad para perder memoria.

La código anterior se puede mejorar usando *manejadores de limpieza*(((manejador, limpieza))).
Eso manejadores se organizan en una pila de la que se pueden insertar o extraer llamando a {pthread_cleanup_push} y {pthread_cleanup_pop}, respectivamente.
...
<4> Al termina la función se extraen todos los manejadores de señal, puesto que ya no hacen falta.
El argumento 0 hace que {pthread_cleanup_pop} no ejecute el manejador de limpieza extraido.
...
El mecanismo de cancelación de hilos descrito funciona razonablemente bien en C, pero no con lenguajes de más alto nivel, como C++, Java o C#.
...
Por ejemplo, en C++, antes de terminar un hilo, deberían ser llamados todos los destructores de los objetos locales, para evitar *fugas de memoria* y de otros recursos, datos sin escribir y otro problemas derivados de tener objetos que no se destruyen adecuadamente.
Lamentablemente, el mecanismo de cancelación de {pthreads} —y el de otras librerías del hilos, como la de Windows API— no sabe hacer nada de eso.
...
Java y C# ha terminado incluyendo también este tipo de *cancelación cooperativa* usando un *_token_ de cancelación*, debido a los problemas que tienen los desarrolladores para recordar usar correctamente la excepción de la *cancelación coordinada*. 
...
El estándar POSIX establece que si se utiliza {linux_fork} en un programa multihilo, el nuevo proceso debe ser creado con un sólo hilo, que será una réplica del que hizo la llamada, así como un duplicado completo del espacio de direcciones del proceso.
...
En el <<_señales_en_sistemas_operativos_posix>> hablamos del uso de las señales(((señal))) como mecanismo de comunicación, pero en general sirven para informar a un proceso de suceso de ciertos eventos.
...
Como hemos visto, las señales que llegan a un proceso pueden ser interceptadas por una función definida por el programador llamada *manejador de señal*(((manejador, limpieza))).

Las señales también son anteriores a los hilos, por lo que cuando aparecieron los hilos se tuvieron que tomar decisiones sobre como iban a encajar ambos conceptos.
Por ejemplo, decidir cuál de los hilos del procesos será interrumpido cuando llegue una señal, para ejecutar el manejador de señales.
...
Es decir, cada hilo puede elegir que señales quiere bloquear para no tener que atenderlas:
...
El hilo será interrumpido no la ha bloqueado.
...
Esto quiere decir que si señal está configurada para ser manejada usando la acción por defecto y dicha acción es terminar, terminará todo el proceso, aunque la señal haya sido dirigida a un hilo en concreto.
...
Eso facilita el diseño del programa para que maneje las señales adecuadamente.
