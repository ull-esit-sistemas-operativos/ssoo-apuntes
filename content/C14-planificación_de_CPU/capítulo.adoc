...
Además, el algoritmo *FCS* sufre el llamado *((efecto convoy))*.
...
. El proceso P1 termina de usar la CPU y pasa a una cola de dispositivo.
...
Esto nos permite llegar a la siguiente conclusión de que en cierto orden de llegada la mayor parte
...
. Cuando la CPU está disponible, se pone *ejecutnado* el proceso de menor ráfaga de CPU.
...
El problema es que todos estos cálculos consumen tiempo de CPU, cuando el planificador de debe ser lo más rápido posible, dado que se ejecuta con mucha frecuencia.

=== Planificación SRTF
(((planificación, SRTF)))

El algoritmo *SJF* es *cooperativo*, pero si se puede implementar de forma *expropiativa*, en cuyo caso se llama *((SRTF))* (Shortest-Remaing-Time First).
...
En la *planificación con prioridades* asocia una prioridad a cada proceso, de tal forma que el de prioridad más alta es asignado a la CPU.
...
En el caso extremo en el que el *cuanto* es tan grande que ningún procesos lo agota, el *RR* se convierte en *FCFS*, que suele tener grandes *tiempos de espera promedio*.
+
...
Sin embargo, eso no va a ocurrir rápidamente si en el sistema hay procesos limitados por la CPU, pues estos generalmente agotan el *cuanto* antes de ser forzados volver a la *cola de preparados*.
...
La *planificación equitativa* resuelve este problema permitiendo que a los procesos se les asignen pesos, repartiendo proporcionalmente más tiempo de CPU a los procesos con mayor peso.
...
image::planificación_colas_multinivel.svg[]
...
+
Por ejemplo, en la <<esquema_colas_multinivel>>, mientras un proceso de prioridad 1 esté preparado ningún otro de clase prioridad inferior.
...
(((planificación, multinivel, realimentada)))
...
image::planificación_colas_multinivel_realimentadas.svg[]
...
Ambas tienen una organización muy diferente pero, en última instancia, las primeras debe mapearse en las segundas.
...
image::administrador_de_tareas.jpg[]
...
Esta prioridad base es la prioridad real del hilo, sí esté tiene una prioridad en el rango de tiempo real —es decir, sí el proceso al que pertenece está en la clase de prioridad `REALTIME_PRIORITY_CLASS`—.
...
Estas bonificaciones se truncan para que nuncan pueden hacer que el hilo se meta en el rango de tiempo real.
...
Se bonifican los hilos que despierta tras completar operaciones de E/S con una cantidad que depende del tipo de dispositivo.
...
En este último caso, incluso se les ofrece más tiempo de *cuanto* si es el hilo asociado a la ventana primer plano, con el objetivo de mejorar la respuesta de las aplicaciones interactivas.
