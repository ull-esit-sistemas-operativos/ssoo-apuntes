...
El modelo *muchos a uno* corresponde al caso en el que solo se implementan *hilos a nivel de usuario*.
...
Realmente, son los mismos *hilos de núcleo* pero vistos desde la perspectiva del programa en el modo usuario.
...
Mientras que el principal *inconveniente* es su complejidad y la dificultad de coordinar el planificador de la librería de hilos en espacio de usuario con el planificador de la CPU para obtener el mejor rendimiento.
...
Por lo general, primero se planifican las segundas, dejando para el final las tareas intensivas en el uso de la CPU.

El problema con el modelo *muchos a muchos* es que en un mismo *hilo de núcleo* se pueden ejecutar diferentes *hilos de usuario* que pueden ser de distinto tipo.
...
La solución pasa porque la *librería de hilos* en espacio de usuario y el planificador de la CPU intercambien información sobre sus hilos y se coordinen.

Por estas dificultades, sistemas como Linux —y {solaris} a partir de la versión 9— han optado finalmente por el modelo *uno a uno*.
En concreto, los desarrolladores de Linux han preferido concentrar sus esfuerzos en conseguir un planificador de CPU más eficiente, así como en reducir los costes de la creación de *hilos de núcleo*, con el objeto de que haya poca o ninguna ventaja en implementar *hilos de usuario*.
...
También Microsoft Windows —a partir de Windows 7— soporta este modelo gracias a incorporar un mecanismo denominado _planificación en modo usuario_ (véase https://docs.microsoft.com/en-us/windows/win32/procthread/user-mode-scheduling[«User-Mode Scheduling -- Microsoft Docs»]).

[NOTE]
====
En Microsoft Windows también se puede implemnentar el modelo *muchos a muchos* usando múltiples *hilos de núcleo* y https://docs.microsoft.com/en-us/windows/win32/procthread/fibers[fibras], aunque es preferible usar _planificación en modo usuario_.

La diferencia es que en la _planificación en modo usuario_ la planificación de hilos es *expropiativa* —es decir, la *librería de hilos* puede interrumpir e intercambiar los hilos en cualquier momento— mientras que las _fibras_ en Windows son *cooperativas* —por lo que deben interrumpir su ejecución voluntariamente para que puedan ser intercambiadas por otra _fibra_—.
====
...
Tanto en el modelo *muchos a muchos* como en el de *dos niveles* es conveniente que exista cierto grado de coordinación entre el núcleo y la *librería de hilos* del espacio de usuario.
