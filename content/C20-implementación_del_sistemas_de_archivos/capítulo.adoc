...
Mientras que las estructuras de metadatos utilizadas, tanto en la memoria como en disco, fueron tratadas brevemente en el <<_estructuras_de_metadatos>>.
...
En {ext4} el espacio se asigna a los archivos en *extensiones* de hasta 128MB, compuestas por bloques, generalmente, de 4KB.
...
En los sistemas de archivos {fat} cada entrada de directorio contiene, a parte del nombre del archivo y otros atributos, el número del primer clúster con datos del archivo.
...
* Los primeros 12 punteros en el *inodo* son directos, seguidos de un puntero indirecto y un puntero doblemente indirecto.
...
Por ejemplo, un disco de 40 GB con bloques de 512 bytes necesitará un mapa de bits de más de 10 MB, lo que no es un gran requisito para un sistema moderno pero si lo era hace dos décadas.
