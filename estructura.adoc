...
. Los componentes del sistema operativo y sus interconexiones (véase el apartado <<_componentes_del_sistema>>).
...
Por ejemplo, en Microsoft Windows, tanto `open()` en Python como `fopen()` en C tiene que llamar internamente la función `CreateFile()` de la librería del sistema (véase la ).
====
...
Sin embargo, el código del núcleo del sistema operativo suele estar en una ubicación diferente que, desde el punto de vista de los programas, no es conocida y generalmente está protegida frente a accesos indebidos (véase el apartado <<_protección_de_la_memoria>>).
...
Por el momento estamos considerando que _proceso_ y _trabajo_ (véase el apartado <<_mainframe>>) hacen referencia al mismo concepto.
Sin embargo más adelante veremos que el segundo es mucho más general que el primero puesto que un proceso puede colaborar con otros procesos para desarrollar un trabajo determinado (véase el apartado <<_operaciones_sobre_los_procesos>>).
